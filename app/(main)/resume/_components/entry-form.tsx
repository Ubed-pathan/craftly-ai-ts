...
export function EntryForm({ type, entries, onChange }: { type: string; entries: Entry[]; onChange: (e: Entry[]) => void }) {
  ...
  setValue("description", improvedContent);
    ...
    if (improveError) {
      toast.error((improveError as any).message || "Failed to improve description");
    }
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                {errors.organization && <p className="text-sm text-red-500">{errors.organization.message as string}</p>}
              </div>
            </div>
