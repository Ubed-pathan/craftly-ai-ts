...
export default function OnboardingForm({ industries }: { industries: Industry[] }) {
  ...
  const { loading: updateLoading, fn: updateUserFn, data: updateResult } = useFetch<Awaited<ReturnType<typeof updateUser>>>(updateUser as any);
  ...
  const {
    ...
    setValue,
    watch,
  } = useForm<OnboardingFormInput, any, OnboardingFormOutput>({ resolver: zodResolver(onboardingSchema) });

  const onSubmit = async (values: OnboardingFormOutput) => {
    try {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="space-y-2">
              ...
              <Select
                onValueChange={(value: string) => {
                  ...
                  const found = industries.find((ind) => ind.id === value) ?? null;
              ...
              {errors.industry && <p className="text-sm text-red-500">{errors.industry.message as string}</p>}
            </div>
            ...
            {watchIndustry && (
              <div className="space-y-2">
                ...
                {errors.subIndustry && <p className="text-sm text-red-500">{errors.subIndustry.message as string}</p>}
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              {errors.experience && <p className="text-sm text-red-500">{errors.experience.message as string}</p>}
            </div>
            ...
            <div className="space-y-2">
              ...
              {errors.bio && <p className="text-sm text-red-500">{errors.bio.message as string}</p>}
            </div>
